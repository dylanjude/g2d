...
# identify file to read
    filename = "{:s}_r{:07.0f}_a{:05.1f}_m{:04.2f}.forces".format(prefix,R,aoa%360,M)
...
def collect_g2d_outputs(directory,airfoil_name,doplot=True):
# collect all outputs
    all_outputs = []
        ...
        for imach,Mach in enumerate(all_Mach):
            for ialpha,alpha in enumerate(all_alpha):
                Cl[ialpha,imach,ire], Cd[ialpha,imach,ire], Cm[ialpha,imach,ire] = \
                    read_g2d_output(prefix,Re,alpha,Mach,plot_history=doplot)
